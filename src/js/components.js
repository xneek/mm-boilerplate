...
const com = {
	...
	*/
	use: function(name, callback){
		...
		return new Promise(function(good, bad){
			...
			var loadOneDep =  function(depss, index, cb1){
					if(!depss){cb1()}
			...
			if( typeof com.list[name] === 'undefined' ){ console.log("Module -s is not defined", name); return false;}
			if( typeof com.list[name].beforeLoad === 'function' ){ com.list[name].beforeLoad(func); }
			...
			loadDependencies(app.modules.list[name].depend || [], function(){
				...
				app.include(app.modules.list[name].files, function(){
					...
					if(typeof app.modules.list[name].afterLoad === 'function'){ app.modules.list[name].afterLoad();}
					if( typeof callback === 'function' ){ callback(func); }
					good.call(func);
				})
			});
	...
	showDocs: function(name){
		if( typeof app.modules.list[name] === 'undefined' ){ console.log("Module -s is not defined", name); return false;}
	...
	*/
	define: function(name, params){
			if( typeof app.modules.list[name] != 'undefined' ){ console.log("Module already exists", name); return false;}
...
app.modules.define('moment', {
	afterLoad: function(){moment.locale('ru');},
	desc:'дата/время',
